...
#define HIDDEN    300
/******************************************************************************/
#define DATA_SET_SIZE 2724       //Number of samples in  dataset
 
//#define DATA_SET_SIZE 8729       //Number of samples in  dataset
#define ATTRIBUTES 388  //Number of pixels per sample.16X16

#define TRAINING_SAMPLES 2000     //Number of samples in test dataset
#define TEST_SAMPLES 724      //Number of samples in test dataset
...
void read_dataset(char *filename, cv::Mat &data, cv::Mat &labels, cv::Mat &labs)
{
    ...
    cout<<"classes: "<<classes.rows<<" classes: "<<classes.cols<<endl;
	...
	for(int i=0 ; i <  classes.rows ; i++)	
	{
		for( int j = 0; j < classes.cols ; j++) 
		{
          ...
          //  case 2 : labels.at<double>(i,3) = 1; break;
           
			default: break;
			}
             cout <<labels.at<double>(i,0)
                    <<labels.at<double>(i,1)
                    <<labels.at<double>(i,2)
                    <<labels.at<double>(i,3)
                    <<labels.at<double>(i,4)
                    <<labels.at<double>(i,5)
                    <<labels.at<double>(i,6)
                    <<labels.at<double>(i,7)
                    <<labels.at<double>(i,8)
                    <<labels.at<double>(i,9)
                    <<labels.at<double>(i,10)
                    <<labels.at<double>(i,11)
                    <<labels.at<double>(i,12)
                    <<endl;
		}
	}
...
double precision(Mat & samples, Mat & samples_classes,  CvANN_MLP& nnetwork, bool show_sample_img = false)
{
    ...
    for (int tsample = 0; tsample < samples.rows; tsample++) {
       ...
       // cout<<"rows: "<<labels.rows<<" cols: "<<labels.cols<<endl;
        ...
        if(tsample+TRAINING_SAMPLES>2720&&show_sample_img)
        cout<< "samples_classes: "<<(int)samples_classes.at<double>(tsample,0)
            <<(int)samples_classes.at<double>(tsample,1)
            <<(int)samples_classes.at<double>(tsample,2)
            <<(int)samples_classes.at<double>(tsample,3)
            <<(int)samples_classes.at<double>(tsample,4)
            <<(int)samples_classes.at<double>(tsample,5)
            <<(int)samples_classes.at<double>(tsample,6)
            <<(int)samples_classes.at<double>(tsample,7)
            <<(int)samples_classes.at<double>(tsample,8)
            <<(int)samples_classes.at<double>(tsample,9)
            <<(int)samples_classes.at<double>(tsample,10)
            <<(int)samples_classes.at<double>(tsample,11)
            <<(int)samples_classes.at<double>(tsample,12)
        <<  " prediction :  " <<  maxIndex+1 << endl;    
    }
...
int main( int argc, char** argv )
{
    ...
    cout<<"rows: "<<dataset388.rows<<" cols: "<<dataset388.cols<<endl;
    ...
    CvANN_MLP_TrainParams params(                                   
                                ...
                                cvTermCriteria(CV_TERMCRIT_ITER+CV_TERMCRIT_EPS, 100, 0.00000001),
