...
Mat CEllipseDetectorYaed::CutROI(Mat &output, std::vector<Ellipse> &ellipses,int iTopN)
{
	Mat result;
	int sizeofEll=int(ellipses.size());
	int n=(iTopN==0)?sizeofEll:min(iTopN,sizeofEll);
	int areaSize=-1;
	int maxA=-1;
	int maxB;
	int indemaxX;
	...
	for(int i=0;i<n;++i){
		Ellipse &e=ellipses[n-i-1];
		alpha=1.0*cvRound(e._a)/cvRound(e._b);
		//cout<<cvRound(e._xc)<<" "<<cvRound(e._yc)<<endl;
		if(alpha>0.95&&alpha<1.02)
			if( cvRound(e._a)*cvRound(e._b)>areaSize ){
				//cout<<areaSize<<endl;
				maxA=cvRound(e._a);
				maxB=cvRound(e._b);
				indemaxX=n-i-1;
				areaSize=cvRound(e._a)*cvRound(e._b);
			}
	}
	if(maxA==-1){
		return result;
	...
	Ellipse &e=ellipses[indemaxX];
	int edgeLength=maxA>maxB?maxA:maxB;
	


	int minX,maxX,minY,maxY;
	minX=cvRound(e._yc)-maxA;
	if(minX<0) minX=0;
	minY=cvRound(e._xc)-maxB;
	if(minY<0) minY=0;
	maxX=cvRound(e._yc)+maxA;
	if(maxX>output.rows) 
		maxX=output.rows;
	maxY=cvRound(e._xc)+maxB;
	if(maxY>output.cols)
		maxY=output.cols;
